...
def load_jsonl(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            try:
                # Load each line as a JSON object and append it to the data 
                # list 
...
def get_style_number(style_name):
    style_dict = {'cinematic': 0, 'cartoon': 1, 'realistic': 2, 'scribble': 3}
    if style_name in style_dict:
        return style_dict[style_name]
    else:
        raise ValueError(f"Style '{style_name}' not found in the dictionary.")

# # ---------------------------------------------------------
# # The following part is borrowed from:
# # Repository: https://github.com/jayleicn/moment_detr/
# # File: blob/main/run_on_video/data_utils.py
# # ---------------------------------------------------------
#
...
#         self.clip_extractor, self.preprocess = clip.load(model_name_or_path, device=device, jit=False)
...
#         encoded_img = self.preprocess(Image.open(image_path)).unsqueeze(0).to(self.device)
#         with torch.no_grad():
#             output = self.clip_extractor.encode_image(encoded_img)
#         img_features = [output]
#         return img_features  # List([L_j, d]) torch tensor
#
#
